...
不等待后续无关插件。Host 先加载独占工具策略 owner 及其依赖，再以最多三个并发任务
加载依赖已经完成的插件；实际完成顺序不会改变计划中的工具和提示顺序。Agent 在
`RunStarted` 时冻结当时 Ready 的工具、提示、路由和能力 owner；运行中新增的插件从
下一轮生效。独占工具策略 owner 尚未 Ready 时，Host 会阻止工具调用，避免加载窗口
弱化权限。
