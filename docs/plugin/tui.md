...
插件版 TUI 会先显示首帧并立即开放 Agent，再在后台按依赖顺序渐进加载插件。加载期间提交普通消息会使用该轮开始时已经 Ready 的工具、提示和路由；本轮执行期间新 Ready 的插件不会改变模型能力，从下一条用户消息起自动可见。Command 尚未 Ready 时，斜杠命令保留在输入框，不会误发给模型。
...
TUI 会在每个插件 Ready 时立即消费其激活事件，并在底部信息栏右侧按加载顺序累积状态，例如 `mcp: MCP 插件等待配置` 和 `skill: 已加载 1 个 Skill`。全部加载结束数秒后该区域收敛为 `◈ N plugins`，持续显示当前已加载插件数量，不额外占用对话区高度。
