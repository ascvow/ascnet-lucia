...
- `agent-tui` 只负责应用组装、配置、输入和渲染。具体插件协议必须通过版本化服务或通用 Host API 交互，禁止在 TUI 复制插件业务规则。
...
- 修改插件 UI 类型时必须更新 `wit/fixtures/ui-v1.json`，并保证 Guest 与 Host 的双边契约测试同时通过；禁止只修改其中一侧。
...
- Command 等关键官方插件必须通过真实 WASM Host smoke test；原生单元测试和 component 编译只能作为前置检查，不能替代端到端路由验证。
