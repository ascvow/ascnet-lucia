...
插件声明一个右侧“团队”摘要视图、一个全屏“团队工作台”和一个成员 Agent 子视图。右侧视图显示队长的等待、分析或工具执行状态，以及成员数、运行数、未确认消息数和每个成员的明确状态标签；聚焦该视图后确认即可进入团队工作台。工作台选择成员后按 `Enter` 进入其主界面，视图约每秒拉取一次 Runtime 状态与事件，通过 Guest SDK 的通用 Agent 视图展示可见模型输出、工具调用和运行状态，不展示隐藏推理内容。
