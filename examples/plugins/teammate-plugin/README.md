...
插件声明一个右侧“团队”摘要视图、一个全屏“团队工作台”和一个成员 Agent 子视图。右侧视图与主输入框底部对齐，使用独立的统计、队长和成员区域展示运行状态；固定在面板底部的入口显示 `Tab → Enter`，聚焦后切换为选中态，鼠标点击也可直接进入团队工作台。工作台选择成员后按 `Enter` 进入其主界面，视图约每秒拉取一次 Runtime 状态与事件，通过 Guest SDK 的通用 Agent 视图展示可见模型输出、工具调用和运行状态，不展示隐藏推理内容。
