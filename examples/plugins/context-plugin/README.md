...
Command 插件通过受控回调调用 `context.compact`，登记一次手动压缩请求。用户发送下一条消息时，Context 插件忽略自动水位并执行完整压缩；同一次 ReAct run 的后续工具步骤继续使用压缩上下文，run 结束后恢复自动水位策略。当前会话没有可安全切分的历史轮次时，主事件列表会提示“没有可压缩的历史上下文”。
